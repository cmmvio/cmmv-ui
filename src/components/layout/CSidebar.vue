<template>
    ...
    >
        ...
        >
            <div
                v-if="isOpen && !absolute"
                class="fixed inset-0 z-40 transition-opacity c-sidebar"
                :class="[roundedStyles[rounded], bgColorOverlay]"
                @click="close"
            ></div>

            <div
                v-else-if="isOpen && absolute"
        ...
        <transition :name="slideTransition">
            <div
                v-if="isOpen"
                class="z-50 flex flex-col transition-all duration-300 overflow-hidden"
                :class="[
            ...
            >
                <slot v-if="$slots.header || $slots.title" name="header">
                    <div class="p-4 flex justify-between items-center border-b border-gray-200 dark:border-neutral-800">
                        ...
                        <button @click="close" class="p-2 rounded-md hover:bg-gray-300 dark:hover:bg-gray-700">
                            <icon-x-mark class="w-6 h-6 text-neutral-600 dark:text-white" size="md" aria-hidden="true" />
                        </button>
                    </div>
                </slot>
                ...
                <slot v-if="$slots.footer || $slots['footer-content']" name="footer">
                    <div class="p-4 border-t border-gray-200 dark:border-black text-center text-sm text-gray-600 dark:text-gray-400">
