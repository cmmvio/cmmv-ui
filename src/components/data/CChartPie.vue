    ...
    const datasets = Array.isArray(props.data.datasets)
        ? props.data.datasets.map((dataset) => {
            const backgroundColor = dataset.backgroundColor ||
                labels.map((_, i) => props.colors[i % props.colors.length]);
            ...
            return {
                ...
                borderWidth: dataset.borderWidth || props.borderWidth,
    ...
    try {
        ...
        newDatasets.forEach((dataset, i) => {
            ...
            if (Array.isArray(dataset.borderColor)) {
                ...
                dataset.borderColor.push(color.replace(/[^,]+(?=\))/, '1'));
            }
        });
