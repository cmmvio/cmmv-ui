...
const props = defineProps({
  ...
  // Tipo de linha (stepped, curved, straight)
  lineType: {
    type: String,
    default: 'straight', // 'straight', 'curved', 'stepped'
    validator: (value) => ['straight', 'curved', 'stepped'].includes(value)
  },
  // Mostrar pontos
  showPoints: {
    type: Boolean,
...
function prepareChartData() {
  ...
  const datasets = Array.isArray(props.data.datasets)
    ? props.data.datasets.map((dataset, index) => {
        ...
        let tension = 0; // linha reta por padrão
        if (props.lineType === 'curved') {
          tension = 0.4; // linha curva
        }

        const stepped = props.lineType === 'stepped';
