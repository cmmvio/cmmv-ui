<template>
    <div v-for="position in positions" :key="position" aria-live="assertive"
        class="pointer-events-none fixed inset-0 flex px-4 py-6 z-50" :class="getPositionClasses(position)">
        <div class="flex flex-col space-y-4 z-50" :style="{ maxWidth: maxWidth ? `${maxWidth}px` : 'auto' }">
            <transition-group name="notification-slide" tag="div" class="flex flex-col space-y-4">
                <div v-for="notification in getNotificationsByPosition(position)" :key="notification.id"
                    class="pointer-events-auto overflow-hidden rounded-lg shadow-lg ring-1 ring-black/5" :class="[
                    ...
                    <div class="p-4">
                        <div class="flex items-start">
                            ...
                            <div class="ml-3 w-0 flex-1">
                                <div v-if="notification.title" class="text-sm font-medium mt-0 pt-0"
                                    :class="notification.titleClass || ''">
                                    {{ notification.title }}
                                </div>
                                <div v-if="notification.content" class="mt-1 text-xs"
                            ...
                            <div class="ml-4 flex shrink-0">
                                <c-button v-if="!notification.permanent" type="button" variant="plain" shadow="none"
                                    @click="() => closeNotification(notification.id)"
                                    class="inline-flex rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                                    <span class="sr-only">Close</span>
                                    <IconXMark class="h-5 w-5 text-neutral-400 dark:text-white hover:text-neutral-500"
                                        aria-hidden="true" />
                                </c-button>
                            </div>
                        </div>

                        <div v-if="!notification.permanent && notification.duration"
                            class="relative mt-3 h-1 w-full rounded"
                            :class="notification.progressBgClass || 'bg-neutral-200 dark:bg-neutral-500'">
                            <div :style="{ width: `${notification.progress}%` }"
