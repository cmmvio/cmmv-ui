		...
		<div class="relative">
			...
			<div class="relative flex items-center z-30" @click="toggleCalendar">
				<input
					...
					:class="[
						disabled ? 'bg-gray-100 dark:bg-neutral-700 cursor-not-allowed' : 'bg-white dark:bg-neutral-800 cursor-pointer',
						hasError ? 'border-red-500 focus:border-red-500' : 'border-neutral-300 dark:border-neutral-600 focus:border-blue-600 dark:focus:border-blue-500',
				...
				>
					<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-6 h-6 text-neutral-800 dark:text-white">
			...
			<transition name="dropdown">
				<div
					v-if="showCalendar"
					ref="calendarRef"
					class="absolute z-50 mt-2 bg-white dark:bg-neutral-800 rounded-md shadow-lg border border-neutral-300 dark:border-neutral-700 p-2 w-72 calendar"
				>
					<div class="flex justify-between items-center mb-2">
						<button
							class="p-1 rounded-full hover:bg-gray-100 dark:hover:bg-neutral-700 transition-colors text-gray-700 dark:text-neutral-300"
						...
						<div class="flex items-center space-x-1">
							<select
								...
								class="appearance-none bg-transparent px-1 py-0.5 rounded hover:bg-gray-100 dark:hover:bg-neutral-700 cursor-pointer text-gray-900 dark:text-white"
								@change.stop="updateCalendar"
								@click.stop
							>
					...
					<div v-if="range" class="flex justify-between mb-2 text-xs text-gray-600 dark:text-neutral-400">
					...
					<div class="grid grid-cols-7 gap-1 mb-1">
						<div
							...
							:key="day"
							class="text-center text-xs font-medium text-gray-500 dark:text-neutral-400 py-1"
						>
							{{ day }}
						</div>
					</div>
					...
					<div class="grid grid-cols-7 gap-1">
						<div
							...
							:class="[
								day.isCurrentMonth && isDateInRange(day.date) ? 'cursor-pointer hover:bg-gray-100 dark:hover:bg-neutral-700' : 'text-gray-400 dark:text-neutral-500 cursor-not-allowed opacity-50',
								...
								!isSelectedDate(day.date) && !isInSelectedRange(day.date) && day.isCurrentMonth && isDateInRange(day.date) ? 'text-gray-900 dark:text-white' : ''
							]"
					...
					<div class="flex justify-between mt-2 pt-2 border-t border-neutral-200 dark:border-neutral-700">
						<button
						...
						<button
							class="text-sm text-gray-600 dark:text-neutral-400 hover:text-gray-800 dark:hover:text-neutral-300 transition-colors"
